...
| S5 | Retrospective prediction check: fit-set mean has lower pooled point-estimate MSE than every fitted predictor for all three features. | [Summary](../evidence/sae_prediction/prediction_summary.csv), [per-case errors](../evidence/sae_prediction/prediction_errors.csv), [recheck script](../scripts/check_prediction_baselines.py). | Constants added after the original analysis; 16 check groups per feature; no refit-aware or population-level superiority claim. |
...
New data and changed endpoints should receive new dated records; frozen evidence should not be silently rewritten.
