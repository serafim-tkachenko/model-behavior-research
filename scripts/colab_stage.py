...
from model_behavior_research.scientific.collect import configuration, collect
from model_behavior_research.scientific.regimes import RegimeConfig
from model_behavior_research.scientific.run import analyze, manifest
...
def main(stage, settings_path):
    ...
    if stage == "sanity":
        ...
        from model_behavior_research.scientific.controls import weak_controls

        weak_controls(cfg, rcfg)
    elif stage == "robustness":
        from model_behavior_research.scientific.robustness import one_per_document

        one_per_document(cfg, rcfg)
    elif stage == "report":
        from model_behavior_research.scientific.report import report

        report(cfg, rcfg)
        manifest(cfg, rcfg)
    elif stage == "export":
        from model_behavior_research.scientific.export import export_bundle
