...
Setup clones the public research repository. Obtain model access before running. A Colab secret named HF_TOKEN is used when available. Drive mounting is optional.
...
REPO = "https://github.com/serafim-tkachenko/model-behavior-research.git"
REF = "main"
ROOT = pathlib.Path("/content/model-behavior-research")
if not (ROOT / "src/model_behavior_research/scientific/run.py").exists():
...
subprocess.run(["uv", "pip", "install", "--python", PY, "-e", "."], check=True)
